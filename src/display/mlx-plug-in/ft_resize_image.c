...
/*   Updated: 2023/10/26 12:40:08 by acarlott         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_resize_img(t_cub3d *cub, t_img **img, float len)
{
	...
	new_img = mlx_new_image(cub->mlx, (len * (*img)->width), \
	(len * (*img)->height));
	if (!new_img)
	...
	while (++y < new_img->height)
	{
		...
		while (x < new_img->width)
		{
			...
			src_offset = ((*img)->width * (int)(y / len)) + (x / len);
			((int *)new_img->data)[dest_offset] = \
			((int *)(*img)->data)[src_offset];
			x++;
		}
	}
